...
TipoCentroMedico.create = (nombre) => {
  return db.none(`INSERT into tipo_centro_medico(nombre)` + `VALUES($1)`, [
    nombre,
  ]);
};
...
TipoCentroMedico.update = (nombre, tipoCentroMedicoId) => {
  return db.none(`UPDATE tipo_centro_medico SET nombre = $1 WHERE id = $2`, [
    nombre,
    tipoCentroMedicoId,
  ]);
};
...
TipoCentroMedico.delete = (tipoCentroMedicoId) => {
  return db.none(
    `DELETE from tipo_centro_medico WHERE id = $1`,
    tipoCentroMedicoId
  );
};
